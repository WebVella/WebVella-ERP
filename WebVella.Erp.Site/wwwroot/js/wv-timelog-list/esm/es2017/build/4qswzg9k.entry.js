import { h } from '../wv-timelog-list.core.js';
...
const ADD_TIMELOG = "ADD_TIMELOG";
const REMOVE_TIMELOG = "REMOVE_TIMELOG";

function addTimelog(payload) { return dispatch => dispatch({ type: ADD_TIMELOG, payload: payload }); }
function removeTimelog(payload) { return dispatch => dispatch({ type: REMOVE_TIMELOG, payload: payload }); }
...
function SubmitReplyForm(scope) {
    ...
    requestBody["loggedOn"] = hooks(scope.loggedOn).local().format('YYYY-MM-DDTHH:mm:ss');
...
class WvAddNew {
    ...
    componentWillLoad() {
        ...
        this.loggedOn = hooks().toISOString();
...
class WvPost {
    ...
    render() {
        ...
        let timeString = hooks(scope.post["created_on"]).format('DD MMM YYYY HH:mm');
        let logString = hooks(scope.post["logged_on"]).format('DD MMM YYYY');
...
const rootReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case REMOVE_TIMELOG:
            {
                let filteredRecords = lodash.filter(newState.records, function (record) { return record.id != action.payload.timelogId; });
...
export { WvAddNew as WvAddTimelog, WvPost as WvTimelog, WvPostList as WvTimelogList };
