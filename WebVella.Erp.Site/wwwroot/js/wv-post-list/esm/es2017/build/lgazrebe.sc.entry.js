import { h } from '../wv-post-list.core.js';
...
function addPost(payload) { return dispatch => dispatch({ type: ADD_POST, payload: payload }); }
function removePost(payload) { return dispatch => dispatch({ type: REMOVE_POST, payload: payload }); }
function addComment(payload) { return dispatch => dispatch({ type: ADD_COMMENT, payload: payload }); }
function removeComment(payload) { return dispatch => dispatch({ type: REMOVE_COMMENT, payload: payload }); }
...
class WvComment {
    ...
    render() {
        ...
        let userName = "system";
        let hrTime = hooks(scope.comment["created_on"]).fromNow();
        let timeString = hooks(scope.comment["created_on"]).format('YYYY MM DD HH:mm');
...
function SubmitReplyForm$1(scope) {
    let storeState = scope.store.getState();
...
class WvPost {
    ...
    render() {
        ...
        let userImagePath = "/assets/avatar.png";
        let userName = "system";
        let hrTime = hooks(scope.post["created_on"]).fromNow();
        let timeString = hooks(scope.post["created_on"]).format('YYYY MM DD HH:mm');
        ...
        case REMOVE_POST:
            {
                switch (action.payload.mode) {
                    default:
                        let filteredPosts = lodash.filter(newState.posts, function (record) { return record.id != action.payload.postId; });
        ...
        case ADD_COMMENT:
            {
                switch (action.payload.mode) {
                    default:
                        var postIndex = lodash.findIndex(newState.posts, function (record) { return record.id === action.payload.comment.parent_id; });
                        if (postIndex === -1) {
        ...
        case REMOVE_COMMENT:
            {
                switch (action.payload.mode) {
                    default:
                        var postIndex = lodash.findIndex(newState.posts, function (record) { return record.id === action.payload.parentId; });
                        if (postIndex === -1) {
                            console.error("postId not found");
                        }
                        else {
                            let filteredPostNodes = lodash.filter(newState.posts[postIndex]["nodes"], function (record) { return record.id !== action.payload.commentId; });
...
class WvPostList {
    ...
    componentWillLoad() {
        ...
        if (extraPluginsArray.length > 0) {
            config.extraPlugins = lodash.join(extraPluginsArray, ",");
        }
        if (removePluginsArray.length > 0) {
            config.removePlugins = lodash.join(removePluginsArray, ",");
        }
...
export { WvAddNew as WvAddPost, WvComment, WvPost, WvPostList };
