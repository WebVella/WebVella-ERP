import { h } from '../wv-datasource-manage.core.js';
...
class WvDsManageStore {
    constructor() {
        ...
        this.pageDatasourceParams = new Array();
    }
}

function setLibrary(payload) { return dispatch => dispatch({ type: SET_LIBRARY, payload: payload }); }
function setDatasource(payload) { return dispatch => dispatch({ type: SET_DATASOURCE, payload: payload }); }
...
class WvDatasourceParamForm {
    ...
    componentWillLoad() {
        ...
        if (!this.store.getState().library || this.store.getState().library.length === 0) {
            ...
            axios$1.get(apiUrl, requestConfig)
...
class WvDatasourceStep1 {
    ...
    render() {
        ...
        if (scope.libraryQueryString) {
            filteredLibrary = lodash.filter(library, function (x) {
...
class WvDatasourceStep2 {
    ...
    render() {
        ...
        let datasource = lodash.find(library, (x) => x["id"] === scope.datasourceId);
        let iconClass = "fa fa-fw fa-database go-purple";
        if (datasource["type"] === 1) {
            iconClass = "fa fa-fw fa-code go-pink";
        }
        ...
        return (h("div", null,
            ...
            datasource["parameters"].map(function (parameter) {
                ...
                let paramIndex = lodash.findIndex(pageDatasourceParams, (x) => x["name"] === parameter["name"]);
                if (paramIndex > -1) {
...
export { WvDatasourceManage, WvDatasourceParamForm, WvDatasourceStep1, WvDatasourceStep2 };
