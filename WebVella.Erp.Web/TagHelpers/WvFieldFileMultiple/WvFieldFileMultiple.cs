...
namespace WebVella.Erp.Web.TagHelpers
{
	...
	public class WvFieldFileMultiple : WvFieldBase
	{
		...
		private List<string> RelatedFilePaths { get; set; } = new List<string>();
		...
		public override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Value != null && Value is List<EntityRecord>)
			{
				...
				foreach (var file in valueRecords)
				{
					...
					RelatedFilePaths.Add((string)file["path"]);
			...
			if (Mode == FieldRenderMode.Form)
			{
				if (Access == FieldAccess.Full || Access == FieldAccess.FullAndCreate)
				{
					...
					output.Content.AppendHtml($"<input type='hidden' id='input-{FieldId}' name='{Name}' value='{String.Join(',', RelatedFilePaths)}' data-path-field-name='{PathFieldName}'/>");
					#endregion
					...
					{
						var scriptContent = FileService.GetEmbeddedTextResource("form.js", "WebVella.Erp.Web.TagHelpers.WvFieldFileMultiple");
