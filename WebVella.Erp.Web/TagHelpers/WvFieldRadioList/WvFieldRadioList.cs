		...
		public override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == FieldRenderMode.Form)
			{
				...
				foreach (var selectOption in Options)
				{
					...
					if (Value != null && (string)Value == selectOption.Value)
					{
			...
			else if (Mode == FieldRenderMode.InlineEdit)
			{
				if (Access == FieldAccess.Full || Access == FieldAccess.FullAndCreate)
				{
					...
					//Edit
					{
						...
						{
							var scriptContent = FileService.GetEmbeddedTextResource("inline-edit.js", "WebVella.Erp.Web.TagHelpers.WvFieldRadioList");
						...
						var scriptTemplate = @"
						$(function(){
							RadioListInlineEditInit(""{{FieldId}}"",""{{Name}}"",""{{EntityName}}"",""{{RecordId}}"",{{ConfigJson}});
						});";
