	...
	public class WvFieldTime : WvFieldBase
	{
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == FieldRenderMode.Form)
			{
				...
				if (Value != null)
				{
					inputReadOnlyValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToAppDate().Value.ToString("HH:mm", Culture);
			...
			else if (Mode == FieldRenderMode.Display)
			{
				if (Value != null)
				{
					var inputReadOnlyValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToAppDate().Value.ToString("HH:mm", Culture);
			...
			else if (Mode == FieldRenderMode.InlineEdit)
			{
				if (Access == FieldAccess.Full || Access == FieldAccess.FullAndCreate)
				{
					...
					if (Value != null)
					{
						readOnlyInputValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToAppDate().Value.ToString("HH:mm", Culture);
