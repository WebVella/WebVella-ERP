	...
	public class WvFieldTime : WvFieldBase
	{
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == FieldRenderMode.Form)
			{
				...
				if (Value != null)
				{
					inputReadOnlyValue = ((DateTime?)localizationService.ConvertUtcToAppDate(Value) ?? DateTime.Now).ToString("HH:mm", Culture);
				...
				if (Access == FieldAccess.Full || Access == FieldAccess.FullAndCreate)
				{
					...
					{
						var scriptContent = FileService.GetEmbeddedTextResource("form.js", "WebVella.Erp.Web.TagHelpers.WvFieldTime");
			...
			else if (Mode == FieldRenderMode.Display)
			{
				if (Value != null)
				{
					var inputReadOnlyValue = ((DateTime?)localizationService.ConvertUtcToAppDate(Value) ?? DateTime.Now).ToString("HH:mm", Culture);
			...
			else if (Mode == FieldRenderMode.InlineEdit)
			{
				if (Access == FieldAccess.Full || Access == FieldAccess.FullAndCreate)
				{
					...
					if (Value != null)
					{
						readOnlyInputValue = ((DateTime?)localizationService.ConvertUtcToAppDate(Value) ?? DateTime.Now).ToString("HH:mm", Culture);
					...
					if (!tagHelperInitialized)
					{
						var scriptContent = FileService.GetEmbeddedTextResource("inline-edit.js", "WebVella.Erp.Web.TagHelpers.WvFieldTime");
						var scriptEl = new TagBuilder("script");
						scriptEl.Attributes.Add("type", "text/javascript");
						scriptEl.InnerHtml.AppendHtml(jsCompressor.Compress(scriptContent));
						output.PostContent.AppendHtml(scriptEl);
					...
					var scriptTemplate = @"
						$(function(){
							TimeInlineEditInit(""{{FieldId}}"",""{{Name}}"",""{{EntityName}}"",""{{RecordId}}"",{{ConfigJson}});
						});";
					scriptTemplate = scriptTemplate.Replace("{{FieldId}}", (FieldId ?? null).ToString());
