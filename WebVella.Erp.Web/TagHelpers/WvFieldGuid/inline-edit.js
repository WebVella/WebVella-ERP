...
function GuidInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config) {
...
function GuidInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function GuidInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function GuidInlineEditInit(fieldId, fieldName, entityName, recordId, config) {
	...
	$(selectors.viewWrapper + " .action.btn").on("click", function (event) {
		...
		GuidInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		GuidInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					GuidInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
				else {
					GuidInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
			},
...
function GuidInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function GuidInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
