...
function PasswordInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config) {
...
function PasswordInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = PasswordInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function PasswordInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = PasswordInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function PasswordInlineEditInit(fieldId, fieldName, entityName, recordId, config) {
	...
	$(selectors.viewWrapper + " .action.btn").on("click", function (event) {
		...
		PasswordInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		PasswordInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					PasswordInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
				else {
					PasswordInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
			},
...
function PasswordInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = PasswordInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function PasswordInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = PasswordInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
