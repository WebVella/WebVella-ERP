...
function UrlInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config) {
...
function UrlInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = UrlInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function UrlInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = UrlInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function UrlInlineEditInit(fieldId, fieldName, entityName, recordId, config) {
	...
	$(selectors.viewWrapper + " .action.btn").on("click", function (event) {
		...
		UrlInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		UrlInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					UrlInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
				else {
					UrlInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
			},
...
function UrlInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = UrlInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function UrlInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = UrlInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
