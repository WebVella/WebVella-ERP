...
function PhoneInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config) {
...
function PhoneInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = PhoneInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function PhoneInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config) {
	var selectors = PhoneInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function PhoneInlineEditInit(fieldId, fieldName, entityName, recordId, config) {
	...
	$(selectors.viewWrapper + " .action.btn").on("click", function (event) {
		...
		PhoneInlineEditPreEnableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		PhoneInlineEditPreDisableCallback(fieldId, fieldName, entityName, recordId, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					PhoneInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
				else {
					PhoneInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config);
				}
			},
...
function PhoneInlineEditInitSuccessCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = PhoneInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
...
function PhoneInlineEditInitErrorCallback(response, fieldId, fieldName, entityName, recordId, config) {
	var selectors = PhoneInlineEditGenerateSelectors(fieldId, fieldName, entityName, recordId, config);
