	...
	public class RecordRelatedRecordCreatePageModel : BaseErpPageModel
	{
		...
		public IActionResult OnPost()
		{
			...
			if (PageContext.HttpContext.Request.Query.ContainsKey("hookKey"))
			{
				...
				try
				{
					var hookInstances = HookManager.GetHookedInstances<IRecordRelatedRecordCreatePageCustomImplHook>(HookKey);

					foreach (IRecordRelatedRecordCreatePageCustomImplHook inst in hookInstances)
					{
					...
					if (string.IsNullOrWhiteSpace(ReturnUrl))
						return Redirect($"/{ErpRequestContext.App.Name}/{ErpRequestContext.SitemapArea.Name}/{ErpRequestContext.SitemapNode.Name}/r/{ErpRequestContext.ParentRecordId}/rl/{ErpRequestContext.RelationId}/r/{PostObject["id"]}");
					else
