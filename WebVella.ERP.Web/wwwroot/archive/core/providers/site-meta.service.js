...
(function () {
    ...
    function service($http, wvAppConstants) {
        ...
        function getUpdateSiteMeta(successCallback, errorCallback) {
            $http({ method: 'GET', url: wvAppConstants.apiSandboxBaseUrl + 'site/meta' }).success(function (data, status, headers, config) { handleSuccessResult(data, status, successCallback, errorCallback); }).error(function (data, status, headers, config) { handleErrorResult(data, status, errorCallback); });
        }
