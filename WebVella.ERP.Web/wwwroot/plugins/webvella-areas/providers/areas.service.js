...
(function () {
    ...
    function service($log, $http, wvAppConstants) {
        ...
        function getAreaByName(areaName, successCallback, errorCallback) {
            ...
            $http({ method: 'GET', url: wvAppConstants.apiSandboxBaseUrl + '/entities/area/byname/sampleName' }).success(function (data, status, headers, config) { handleSuccessResult(data, status, successCallback); }).error(function (data, status, headers, config) { handleErrorResult(data, status, errorCallback); });
        }
